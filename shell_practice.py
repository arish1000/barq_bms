...
for bank in banks:
    ...
    for branch_data in branches_data:
        Branch.objects.create(bank=bank, **branch_data)

# Filter
islamic_banks = Bank.objects.filter(is_islamic=True)
non_islamic_banks = Bank.objects.filter(is_islamic=False)
meezan_islamic_bank = Bank.objects.filter(is_islamic=True, name="Meezan Bank")

# Filter Islamic banks with active accounts
banks = Bank.objects.filter(is_islamic=True, branches__bank_accounts__is_active=True)
...
accounts = Account.objects.select_related("user", "branch")

# Prefetch_related
banks = Bank.objects.prefetch_related("branches")
banks = Bank.objects.prefetch_related("branches", "branches__bank_accounts")
users = User.objects.prefetch_related("user_accounts")

# Annotate
banks = Bank.objects.annotate(
    total_accounts=Count("branches__bank_accounts")
)
